...
class TextualUI(App):
    ...
    def compose (self) -> ComposeResult:
        ...
        with Vertical (id="menu-container"):
            ...
            yield Button ("Tournamnet", id = "tournament", variant ="primary")
    ...
    def open_tournament (self):
        if self.current_file and os.path.exists(self.set_current_file):
            self.tournament = Tournamnent("Tournament")
            self.tournament.load_from_json(self.current_file)
            self.tournament.set_group_stage()
            self.notify(f"Tournament {self.tournament.name} loaded succefully")
        else:
    ...
    def display_groups(self):
        if self.tournament:
            self.notify ("Here goes the group screen")
            #self.push_screen(GameScreen(self.tournament))
        else:
            self.notify("No tournament loaded")

    def display_games(self):
        if self.tournament:
            self.notify ("Here goes the group games")
            #self.push_screen(GameScreen(self.tournament))
        else:
            self.notify("No tournament loaded")
    ...
    def play_games (self):
        if self.tournament:
            self.push_screen(PlayGameScreen(self.tournament))
        else:
            self.notify("No tournament loaded")
